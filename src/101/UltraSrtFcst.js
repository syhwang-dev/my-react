...
const UltraSrtFcst = () => {
    ...
    const opt = ultraInfo.map((item, idx) =>
        // <option value={item["항목명"]} key={item["항목값"+idx]}>{item["항목명"]}({item["항목값"]})</option>
        <option value={`${item["항목명"]}:${item["항목값"]}`} key={item["항목값"+idx]}>{item["항목명"]}({item["항목값"]})</option>
        ...
        <article>
            <div className="grid">
                ...
                <div>
                    ...
                    <select value={optionValues} onChange={(e) => setOptionValue(e.target.value)}>
            ...
            {datas && <FcstTable datas={datas} optionValue={optionValues} ultraInfo={ultraInfo} gubun="초단기예보" />}
